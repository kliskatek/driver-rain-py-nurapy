...
class Packet:
    ...
    def __init__(self, command_code: CommandCode, flags: int = 0, *args):
        self.command_buffer = bytearray()
        self.command_buffer.append(Packet.START_BYTE)
        self.command_buffer += int(0).to_bytes(2, 'big')  # Fake length
        self.command_buffer += int(flags).to_bytes(2, 'big')  # Flag word
        ...
        crc16 = self._crc16(self.command_buffer[4:])
        self.command_buffer += int(crc16).to_bytes(2, 'big')

        # Set real command payload length (without 6byte header)
        self.command_buffer[1:3] = (len(self.command_buffer) - 6).to_bytes(2, 'big')
